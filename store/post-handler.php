    ...
    if ($_REQUEST['action'] == 'REGISTRATION')
    {
        if (( $_POST['firstname'] ) && ( $_POST['email'] ) && ( $_POST['password'] ) && ( $_POST['confirmpassword'] ))
        {
            ...
            if (( $_POST['password'] == $_POST['confirmpassword']))
            {
				...
				$message = '<div><p>Dear <strong>'.$_POST["firstname"].' '.$_POST["lastname"].'</strong>, </p><p>Congratulations! You have successfully created a new account with Bridge Store. Your account details are: </p><p>Registered email ID: '.$user_email.' </p><p>Password: <strong>'.$user_pwd.'</strong></p><p>Start login and enjoy your shopping experience with Bridge Store.</p><p><a href="'.$web_root.'index.php?page=login" >'.$web_root.'index.php?page=login</a></p><p>Best Regards,</p><p>Bridge Store Team.</p>';
                ...
                if ($appObj->send_email($mail_info, $subject, $message))
                {
					...
					$user->redirect("index.php?page=registration&confirm=1");	
                }
