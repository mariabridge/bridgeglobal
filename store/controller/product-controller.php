...
public function imageExitence($id)
{
    ...
    public function generate_download_link($purchase_id, $emails, $product_id = null, $base_url = '')
    {
        ...
        if ($product_id)
        {
            ...
            if (!empty($product_info))
            {
					...
					$mail_info = array(
						'from_email'  => 'info@bridge-global.com',
						'from_name'   => 'Bridge Team',
						'reply_email' => 'info@bridge-global.com',
						'reply_name'  => 'Bridge Team',
						'to_email'    => $emails
					);
					$subject       = 'Bridge Store -  Product download Link';
               $download_link = $base_url . 'index.php?page=payment_history#'.$purchase_id;
                ob_start();
	...
	public function sendMail($rand_password)
	{
			...
			$mail_info = array(
						'from_email'  => 'team@bridge-india.in',
						'from_name'   => 'Bridge Team',
						'reply_email' => '',
						'reply_name'  => 'Bridge Team',
						'to_email'    => $emails
				...
				$subject = 'Bridge Store - Password Reset';
  ...
  /**
	...
	public function sendMail_welcome($data)
	{
				...
				$subject = 'Bridge Store - Account Created!';
			...
			include_once 'controller/application-controller.php';
				$appObj = new AppController();
				$message = '<div style="line-height:20px;"><div>Dear Customer, </div><br><div>Congratulations! You have successfully created a new account with Bridge Store. Your account details are: </div><br><div>Registered email ID: '.$user_email.' </div></br><div>Password: <strong>'.$user_pwd.'</strong></div><br><div>Start login and enjoy your shopping experience with Bridge Store.</div><br><div><a href="'.$web_root.'" ><input type="button" value="Start Buying Products" style="color: #fff;background-color: #5bc0de;border-color: #46b8da;"></a></div>';
