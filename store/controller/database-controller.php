...
//        if (mysql_num_rows($result) > 0)
     ...
     * @param int $id
     */
    public function product_image_check_id($id)
    {
        $query  = "SELECT image_path FROM " . $this->db_table_prefix . "products WHERE id = $id";
        $result = $this->commonDatabaseAction($query);
//        if (mysql_num_rows($result) > 0)
        if ($this->rowCount > 0)
        {
...
public function force_user_registration($first_name, $last_name, $email, $username, $password)
{
    $query = "INSERT INTO " . $this->db_table_prefix . "users(username, password, firstname, lastname, email, role_id) VALUES('" . $username . "','" . md5($password) . "','" . $first_name . "','" . $last_name . "','" . $email . "', 2 )";
    $this->commonDatabaseAction($query);
    if ($this->sqlAffected > 0)
        {
            return $this->sqlInsertId();
...
//        if (@@mysql_affected_rows($result) > 0)
    ...
    {
         $query = "SELECT * FROM " . $this->db_table_prefix . "categories Where status=1";
...
public function product_image_insert($product_id, $image_path)
{
        $query = "INSERT into bs_images(`id`,`product_id`,`image_path`) 
                    VALUES(NULL,'$product_id','$image_path');";
...
/**Remove product images by product id**/
