...
// Registartion
    ...
    if (isset($_SESSION ['user_id']) && ( $_SESSION ['user_id'] == '' )) {
        ...
        if ($_SESSION ['user_role'] == 1) {
            ...
            include_once 'templates/home.php&confirm=1';
        }
    }
}
// Buy It Now
    ...
    $product = new ProductController();
        ...
        if(!empty($_SESSION['user_id']))
        {
             ...
             if (isset($_GET['product_id']) && $_GET['page'] == 'addtocart') {
						 ...
						 $cart_products = $product->get_cartItems($_SESSION['user_id']);
                 
                 foreach($cart_products as $cartprd)
                 {
                      if(in_array($_GET['product_id'], $cartprd))
                        {
                         $_SESSION['warning']='Item already added to cart.';
                            $user->redirect('index.php?page=addtocart');
		            exit;
...
// Paypal Response success
    ...
    if (isset($_GET['token']) && $_GET['token'] != '') {
        ...
        if ($is_valid_token) {
            ...
            echo '<p class="error_msg" style="font-size:14px;">Invalid Token. Product download validity got expired</p>';
        }
    }
...
else if($current_file_name == 'customer_delete')
{
        ...
        if(isset($tpages) && isset($pageNo))
                
        $user->redirect('index.php?page=customers&validate_msg=1&tpages='.$tpages.'&pageNo='.$pageNo);
        else
        ...
        $user->redirect('index.php?page=customers&validate_msg=2&tpages='.$tpages.'&pageNo='.$pageNo);
        else
